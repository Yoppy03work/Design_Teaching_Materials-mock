...
// 作品ライブラリ。filter='sample' は本時(STEP6)、'all' は第5時で全作品。
export function WorkGallery({ filter = "sample" }: { filter?: WorkFilter }) {
  const { works, commentsByWork, loading, addComment } = useWorks();
  const shown =
    filter === "sample" ? works.filter((w) => w.type === "sample") : works;

  if (loading) {
    return <p className="text-sm text-muted-foreground">読み込み中…</p>;
  }
  if (shown.length === 0) {
    return <p className="text-sm text-muted-foreground">作品がまだありません。</p>;
  }
